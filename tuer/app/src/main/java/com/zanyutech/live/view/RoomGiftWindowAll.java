...
import static com.zanyutech.live.view.ConstantsString.TAG;
                ...
                if (model != null) {
                    if (model.getCode().equals("1")) {
                        ...
                        if (myGridViewAdapterpk1 != null) {
                            ...
                            Log.e("送背包礼物成功", "1   model.getData().getAttachArray().size()="+model.getData().getAttachArray().size()+"   giftnums="+giftnums+"  needdelnums="+needdelnums+" Sendgiftid="+Sendgiftid);
                        }
                        if(myGridViewAdapterpk2!=null){
                            int needdelnums = model.getData().getAttachArray().size() * Integer.parseInt(giftnums);//   送礼的数量= 送礼人数数组的id数量 X giftnums选择的礼物数量
                            myGridViewAdapterpk2.ChangeCount(Sendgiftid, needdelnums);//重置礼物数字  参数 礼物ID  送出的礼物数量
                            Log.e("送背包礼物成功", "2   model.getData().getAttachArray().size()="+model.getData().getAttachArray().size()+"   giftnums="+giftnums+"  needdelnums="+needdelnums+" Sendgiftid="+Sendgiftid);
                        }
